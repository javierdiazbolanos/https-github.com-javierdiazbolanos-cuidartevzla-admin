...
export default function TickerBar() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("cuidarte_alertas");
    if (saved) {
      try {
      ...
      setAlertas(INITIAL_ALERTAS);
      localStorage.setItem("cuidarte_alertas", JSON.stringify(INITIAL_ALERTAS));
    }
  }, []);

  const saveAlerts = (updated: Alerta[]) => {
    setAlertas(updated);
    localStorage.setItem("cuidarte_alertas", JSON.stringify(updated));
  };
  ...
  const handleAddAlerta = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      saveAlerts(updated);
      // Reset index to show the newest alert
      setCurrentIndex(0);
    }
  ...
  const handlePrev = () => {
    ...
    setCurrentIndex((prev) => (prev === 0 ? alertas.length - 1 : prev - 1));
  };

  const handleNext = () => {
    if (alertas.length === 0) return;
    setCurrentIndex((prev) => (prev === alertas.length - 1 ? 0 : prev + 1));
  };
  ...
  return (
    <div className="space-y-3">
      {/* Pager & Alerts banner (exactly matching the repository theme but with admin tools) */}
      <div className="bg-rose-50 border border-rose-100 rounded-2xl p-3 px-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3 text-left">
          {/* Left status & Alert details */}
          ...
          {/* Right paging navigation and administration */}
          <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
            {/* Nav Pager pill (e.g. < 1/4 >) */}
            {alertas.length > 0 && (
              <div className="bg-white border border-rose-100 rounded-xl px-2 py-1 flex items-center space-x-3 shadow-xs font-mono text-xs text-rose-950 select-none">
                <button
                  type="button"
                  onClick={handlePrev}
                  className="hover:bg-rose-50 p-1 rounded-lg transition-colors text-rose-700 hover:text-rose-950 cursor-pointer"
                  title="Anterior"
                >
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <span className="font-bold tracking-tight">
                  {currentIndex + 1} / {alertas.length}
                </span>
                <button
                  type="button"
                  onClick={handleNext}
            ...
            <button
              ...
              className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-3.5 py-2 rounded-xl flex items-center space-x-1.5 shadow-sm transition-all cursor-pointer hover:shadow-md active:scale-98"
            >
          ...
          <div className="p-5 bg-white border border-slate-200 rounded-2xl animate-fadeIn relative text-left text-slate-900 shadow-sm">
            ...
            <form onSubmit={handleAddAlerta} className="max-w-4xl mx-auto space-y-4">
              <h3 className="text-sm font-bold text-slate-800 flex items-center space-x-2">
                <Megaphone className={`w-4 h-4 ${editingId ? "text-amber-500" : "text-red-600"}`} />
                <span>
                  {editingId 
                    ? "Modificar Alerta de Operaciones Especiales" 
                    : "Nueva Alerta de Operaciones Especiales (Sismo 2026)"}
                </span>
                {editingId && (
                  <span className="bg-amber-100 text-amber-800 text-[10px] px-2.5 py-0.5 rounded-full font-mono font-bold">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-3">
                  <label className="block text-[11px] font-mono text-slate-500 uppercase tracking-wider mb-1.5 font-bold">
                    Texto de la Alerta (Breve, accionable)
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="Ej: Hospital Domingo Luciani al 90% de capacidad pediátrica. Recomendar desvío."
                    value={newTexto}
                ...
                <div>
                  ...
                  <select
                    value={newSeveridad}
                    onChange={(e) => setNewSeveridad(e.target.value as any)}
              ...
              <div className="flex justify-between items-center pt-2 gap-2 flex-wrap">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    type="submit"
                    className={`text-white font-bold px-4 py-2 rounded-xl text-xs flex items-center space-x-1.5 transition-colors cursor-pointer shadow-sm ${
                      editingId ? "bg-amber-600 hover:bg-amber-500" : "bg-red-600 hover:bg-red-500"
                    }`}
                  >
              ...
              {/* List of custom alerts with option to edit or delete */}
              {alertas.length > 0 && (
                      ...
                      <div key={a.id} className="flex justify-between items-center bg-slate-50 hover:bg-slate-100 p-2.5 rounded-xl border border-slate-200 text-xs text-slate-700">
                        <div className="flex items-center space-x-2.5 min-w-0">
                          <span className={`w-2 h-2 rounded-full shrink-0 ${
                            a.severidad === "critica" ? "bg-rose-500 animate-pulse" :
                            a.severidad === "alta" ? "bg-orange-500" :
                            a.severidad === "media" ? "bg-yellow-500" : "bg-sky-500"
                          }`} />
                          ...
                          <span className="text-[10px] font-mono text-rose-500 shrink-0">({a.voluntario})</span>
                        </div>
                        <div className="flex items-center space-x-1 shrink-0 ml-4">
                          <button
                            ...
                            className="text-slate-500 hover:text-sky-600 p-1.5 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="text-rose-600 hover:text-rose-800 p-1.5 rounded-lg hover:bg-slate-200 transition-colors cursor-pointer"
