...
export default function IndividualPatient({ hospitales, onPatientMutated, authorized }: IndividualPatientProps) {
  ...
  const [actualizacionFecha, setActualizacionFecha] = useState(new Date().toISOString().split("T")[0]);
  ...
  const handleCreatePatient = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        actualizacion_fecha: actualizacionFecha,
        pacientes: [
          {
            nombre: homologateName(nombre),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setActualizacionFecha(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 focus:border-sky-500 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none font-mono"
              />
            </div>
              ...
              {searchResults.map((p) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-1 text-xs text-slate-550 mt-2 font-sans">
                    ...
                    <div>
                      Actualización: <span className="text-slate-700 font-mono">{p.actualizacion_fecha || "Sin fecha"}</span>
                    </div>
                  </div>
